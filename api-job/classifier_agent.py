...
class ClassifierAgent:
    ...
    def get_job_score(self):
        ...
        """
        # Get the context
        template_path = Path(__file__).parent / "template_prompt" / "context_classifier.txt"
        with open(template_path, 'r', encoding='utf-8') as f:
            context = f.read()

        # Get the user_prompt
        prompt_path = Path(__file__).parent / "template_prompt" / "prompt_classifier.txt"
        with open(prompt_path, 'r', encoding='utf-8') as f:
            user_prompt = f.read()

        # Get the Groq API response:
        scored_jobs = []
        for job in self.job_offer_list:
            job = select_from_job(job)
            distance = self.get_best_travel_time(job, self.parsed_resume)
            get_score = self.client.chat.completions.create(
                messages = [
                    {"role": "system", "content": context},
                    {"role": "user", "content": user_prompt+json.dumps(self.parsed_resume)+json.dumps(job)+json.dumps(distance)},
            ],
